...
from usdaeco_buildup.profiles import apply_profile, load_profile
...
def test_validator_accepts_profile(tmp_path):
    from usdaeco_buildup import register_plugins, validators
    from pxr import Usd
    register_plugins()
    ...
    path.write_text('{"include_builtin": false, "severity_overrides": {"buildUpArrayLengths": "info"}}')
    issues = validators.validate_stage(stage, profile=path)
    matching = [e for e in issues if e.GetName() == "BuildUpArrayLengths"]
