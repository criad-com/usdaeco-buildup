...
def load_profile(profile):
    ...
    # Retain v0.1 profiles while publishing ProperCase error names in v0.2.
    aliases = {"buildUpArrayLengths": "BuildUpArrayLengths",
               "buildUpTotalMismatch": "BuildUpTotalMismatch"}
    normalized = {aliases.get(name, name): value for name, value in overrides.items()}
    for name in aliases.values():
        if name in overrides:
            normalized[name] = overrides[name]
    return {**data, "severity_overrides": normalized}
